...
def get_or_create_user(username, email, password, full_name, role, phone=None):
...
def run():
    ...
    get_or_create_user("admin", "admin@hostel-demo.com", "admin123", "Ananya Rao", RoleEnum.admin, "9000000001")
    get_or_create_user("warden1", "warden1@hostel-demo.com", "warden123", "Suresh Kumar", RoleEnum.warden, "9000000002")
    ...
    # Keyed by username instead of positional list indexing, so this script
    # is safe to re-run against a database that already has some (but not
    # all) of this data in it.
    students_by_username = {}
    for username, email, pw, name, reg, dept, year in students_data:
        user = get_or_create_user(username, email, pw, name, RoleEnum.student)
